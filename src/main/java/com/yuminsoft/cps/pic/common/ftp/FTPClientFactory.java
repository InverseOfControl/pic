...
public class FTPClientFactory implements KeyedPooledObjectFactory<FTPClientConfigure, FTPClient> {
	...
	public PooledObject<FTPClient> makeObject(FTPClientConfigure key) throws Exception {
		...
		log.debug("connecting " + key.toString() + " used time :" + connectUsed + " ms.");
		// 测试连接是否可用
		int reply = c.getReplyCode();
		if (!FTPReply.isPositiveCompletion(reply)) {
			c.disconnect();
			log.error("FTP链接失败," + key.toString());
			return null;
		}
		...
		log.debug("login " + key.toString() + " used time :" + loginUsed + " ms.");
		if (loginSuccess) {
			log.debug("FTP 登陆成功 : " + key.toString());
		} else {
			log.error("FTP 登陆失败 : " + key.toString());
			return null;
		}
