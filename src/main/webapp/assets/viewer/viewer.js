...
(function (factory) {
  ...
  // Selectors
  var SELECTOR_IMG = 'img:not([data-none])';
  ...
  //扩展
  var EVENT_ROTATELEFT='rotateLeft.' + NAMESPACE;//左旋转
  var EVENT_ROTATERIGHT='rotateRight.' + NAMESPACE;//右旋转
  var EVENT_PREV = 'prev.' + NAMESPACE;
  var EVENT_NEXT = 'next.' + NAMESPACE;
  ...
  Viewer.prototype = {
        ...
        list.push(
          '<li>' +
            '<img' +
              ...
              ' data-action="view"' +
              ' data-index="' +  i + '"' +
              ' data-original-url="' +  ($(this).data('url') || src) + '"' +
              ' alt="' +  alt + '"' +
              ' data-value="' +  value + '"' +
            '>' +
          '</li>'
    ...
    initImage: function (callback) {
      ...
      var viewerHeight = max(viewer.height, footerHeight);
    ...
    renderImage: function (callback) {
      var image = this.image;
      var $image = this.$image;
      //update 2017/7/7 10:14 ym10138  添加弹图右偏移问题
      $image.css({
        width: image.width,
        height: image.height,
        marginLeft: image.left-97,
        marginTop: image.top,
    ...
    loadImage: function (e) {
      ...
      getImageSize(image, function (naturalWidth, naturalHeight) {
        ...
        $image.css({
          width: width,
          height: height,
          marginLeft: (parentWidth - width-(layout=='left'? 194 : 0)) / 2,
    ...
    show: function () {
      ...
      var __index=0;
      var __id=this.target.id;
      // console.log(this.$images[0]);
      //修改了 this.target 内部结构造成 this.$images.index(this.target) 返回索引号找不到.
      this.$images.each(function(i,that){
    	  if(that.id==__id)
    		  __index=i;
      });
      //console.log(__index);
      this.$element.one(EVENT_SHOWN, $.proxy(function () {
        ...
        this.view(this.target ? __index : this.index);
        this.target = false;
      }, this));
    ...
    view: function (index) {
      ...
      if(url.indexOf('picrandom=')==-1){
    	  url=url+separator+'picrandom='+Math.uuid(6);
      }else{
    	  url=url.substring(0,url.indexOf('picrandom='))+'picrandom='+Math.uuid(6)
      }*/
      ...
      this.$element.one(EVENT_VIEWED, $.proxy(function () {
        ...
        if(image && image.naturalWidth && image.naturalHeight){
        	info+=image.naturalWidth + ' &times; ';
        	info+=image.naturalHeight;
        }
        info+=')';
        $title.html(alt + info);
      }, this));
    ...
    prev: function () {
      ...
      if ($.isFunction(this.options.prev)) {
          this.$element.one(EVENT_PREV,this, this.options.prev);
          this.trigger(EVENT_PREV);
      }
    },
    ...
    next: function () {
      ...
      if ($.isFunction(this.options.next)) {
          this.$element.one(EVENT_NEXT,this, this.options.next);
    ...
    rotate: function (degree) {
      // 扩展，禁止CSS控制旋转，采用后端旋转重新加载的方式
      //this.rotateTo((this.image.rotate || 0) + num(degree));
      
      //旋转回调函数扩展
      var options = this.options;
      if (-90==degree && $.isFunction(options.rotateLeft)) {
          this.$element.one(EVENT_ROTATELEFT,this.cacheImg, options.rotateLeft);
          this.trigger(EVENT_ROTATELEFT);
      }
      if (90==degree && $.isFunction(options.rotateRight)) {
          this.$element.one(EVENT_ROTATERIGHT,this.cacheImg, options.rotateRight);
          this.trigger(EVENT_ROTATERIGHT);
      }
    },
        ...
        if (this.isShown) {
          if (this.length) {
            if (this.isViewed) {
              ...
              if (index >= 0) {
                this.isViewed = false;
                this.view(max(this.index - (index), 0));
              } else {
  ...
  Viewer.DEFAULTS = {
    // Enable inline mode 启用 inline 模式
    inline: false,

    // Show the button on the top-right of the viewer  显示右上角关闭按钮（jQuery 版本无效）
    button: true,

    // Show the navbar 显示缩略图导航
    navbar: true,

    // Show the title 显示当前图片的标题（alt 属性及图片尺寸）
    title: true,

    // Show the toolbar 显示工具栏
    toolbar: true,

    // Show the tooltip with image ratio (percentage) when zoom in or zoom out 	显示缩放百分比
    tooltip: true,

    // Enable to move the image  图片是否可移动
    movable: true,

    // Enable to zoom the image 图片是否可缩放
    zoomable: true,

    // Enable to rotate the image 图片是否可旋转
    rotatable: true,

    // Enable to scale the image 图片是否可翻转
    scalable: true,

    // Enable CSS3 Transition for some special elements 使用 CSS3 过度
    transition: true,

    // Enable to request fullscreen when play 播放时是否全屏
    fullscreen: true,

    // Enable keyboard support 是否支持键盘
    keyboard: true,

    // Define interval of each image when playing 播放间隔，单位为毫秒
    interval: 5000,
    ...
    // Define the ratio when zoom the image by wheeling mouse 鼠标滚动时的缩放比例
    zoomRatio: 0.1,

    // Define the min ratio of the image when zoom out 最小缩放比例
    minZoomRatio: 0.01,

    // Define the max ratio of the image when zoom in 最大缩放比例
    maxZoomRatio: 100,

    // Define the CSS `z-index` value of viewer in modal mode. 设置图片查看器 modal 模式时的 z-index
    zIndex: 2015,

    // Define the CSS `z-index` value of viewer in inline mode. 设置图片查看器 inline 模式时的 z-index
    zIndexInline: 0,

    // Define where to get the original image URL for viewing
    // Type: String (an image attribute) or Function (should return an image URL) 设置大图片的 url
    url: 'src',
